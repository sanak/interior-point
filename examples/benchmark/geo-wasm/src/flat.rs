...
//!
//! A copy of `rs/wasm/src/flat.rs`. `geo-wasm` carries its own `[workspace]`, so a path dependency
//! cannot reach the other crate; `js/` is a symlink instead, which keeps the encoder single-sourced.
